...
主机可以安全地升级其状态机的某些部分而不中断IBC功能。为了安全地做到这一点，IBC处理程序逻辑必须保持与规范一致，所有与IBC相关的状态（在可证明和私有存储中）必须在升级过程中保持不变。如果一个升级链的客户存在于其他链上，而升级将改变轻型客户的验证算法，则必须在升级前通知这些客户，以便他们能够安全地进行原子化切换，并保持连接和通道的连续性。
